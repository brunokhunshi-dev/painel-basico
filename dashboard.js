...
async function resolveFirebaseConfig() {
    ...
    for (const candidate of candidates) {
        ...
        } catch (error) { console.warn(`[Dashboard] Não foi possível usar ${candidate.label}:`, error); }
    }
...
function normalizeText(value) { return safeString(value).normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase(); }
...
function parseInputDate(value, endOfDay = false) {
    ...
    const date = new Date(`${value}${suffix}`); return Number.isFinite(date.getTime()) ? date : null;
}
...
function formatDateTime(value) {
    const date = toDate(value); if (!date) return "Não informado";
...
function minutesBetween(start, end) {
    const startDate = toDate(start); const endDate = toDate(end);
...
function formatNumber(value) { return new Intl.NumberFormat(DASHBOARD_CONFIG.locale).format(Number(value) || 0); }
...
function parseCoordinates(value) {
    ...
    if (typeof value === "string") { const parts = value.split(/[;,]/).map(part => Number(part.trim())); [latitude, longitude] = parts; } 
    else if (typeof value === "object") { latitude = Number(value.latitude ?? value.lat ?? value._lat); longitude = Number(value.longitude ?? value.lng ?? value.lon ?? value._long); }
    if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) return null;
    if (Math.abs(latitude) > 90 || Math.abs(longitude) > 180) return null;
    return { lat: latitude, lng: longitude };
}
function initials(name) { return safeString(name, "US").split(/\s+/).slice(0, 2).map(part => part[0] || "").join("").toUpperCase(); }
function firstAvailable(source, keys, fallback = "") {
    for (const key of keys) { const value = source?.[key]; if (value !== undefined && value !== null && safeString(value)) return value; } return fallback;
}
...
function showToast(message, type = "") {
    const toast = $("dashboard-toast"); clearTimeout(state.toastTimer);
    toast.textContent = message; toast.className = `toast ${type ? `is-${type}` : ""}`.trim();
    toast.hidden = false; state.toastTimer = setTimeout(() => { toast.hidden = true; }, 4300);
}

function errorMessage(error) {
    const code = safeString(error?.code); const rawMessage = safeString(error?.message);
    const messages = {
...
function showLogin() { $("dashboard-login").hidden = false; $("dashboard-shell").hidden = true; $("dashboard-btn-entrar").disabled = false; $("dashboard-btn-entrar").textContent = "Entrar"; setLoading(false); clearInterval(state.autoRefreshTimer); state.autoRefreshTimer = null; }
function showDashboard() { $("dashboard-login").hidden = true; $("dashboard-shell").hidden = false; }
...
async function resolveUserAccess(user) {
    ...
    let profile = null;
    for (const collectionName of collectionOrder) { profile = await safeGetProfileFromCollection(collectionName, user); if (profile) break; }

    const email = normalizeText(user.email);
    const allowlisted = DASHBOARD_CONFIG.administratorEmails.some(item => normalizeText(item) === email);

    if (!profile && allowlisted) { profile = normalizeProfessional({ nome: user.displayName || "Gestor", email: user.email, cargo: "Gestão" }, user.uid, DASHBOARD_CONFIG.collections.managers); }
...
async function loadReferenceData(sessionVersion) {
    ...
    if (!professionals.some(person => person.id === state.profile.id)) professionals.push(state.profile);
...
function populateProfessionalFilter() {
    const select = $("filtro-profissional"); const currentValue = select.value; select.replaceChildren();
    const allOption = document.createElement("option"); allOption.value = ""; allOption.textContent = state.canSeeAll ? "Todos os profissionais" : state.profile.name; select.appendChild(allOption);
    if (state.canSeeAll) {
        for (const p of state.professionals) { const opt = document.createElement("option"); opt.value = p.id; opt.textContent = `${p.name} — ${p.role}`; select.appendChild(opt); }
        select.disabled = false; if ([...select.options].some(o => o.value === currentValue)) select.value = currentValue;
...
function populateFormSelects() {
    ...
    if (selCliente) {
        ...
        state.clients.forEach(c => { selCliente.innerHTML += `<option value="${c.id}">${c.name} - ${c.city || ''}</option>`; });
    }
    const selResp = $("v-responsavel");
    if (selResp) {
        selResp.innerHTML = '<option value="">Selecione um técnico...</option>';
        state.professionals.forEach(p => { if(p.ativo) selResp.innerHTML += `<option value="${p.id}">[${p.role}] ${p.name}</option>`; });
    }
}
...
async function loadMissingClients(activities, sessionVersion) {
    ...
    if (sessionVersion !== state.sessionVersion) return;
    for (const r of results) { if (r.status === "fulfilled" && r.value) state.clientMap.set(r.value.id, r.value); }
}

function selectedDateRange() {
    const startValue = $("filtro-data-inicial").value; const endValue = $("filtro-data-final").value;
    const start = parseInputDate(startValue, false); const end = parseInputDate(endValue, true);
...
async function loadActivities({ silent = false } = {}) {
    ...
    const range = selectedDateRange(); state.activitiesLoading = true; const sessionVersion = state.sessionVersion;

    if (!silent) setLoading(true, "Carregando atividades...");
    setSyncStatus("loading", "Atualizando dados...");

    try {
        let activityQuery = state.canSeeAll ? query(collection(db, DASHBOARD_CONFIG.collections.activities), where("data", ">=", range.start), where("data", "<=", range.end)) : query(collection(db, DASHBOARD_CONFIG.collections.activities), where("ptvId", "==", state.profile.id));
        let snapshot;
        try { snapshot = await getDocs(activityQuery); } catch (error) {
            if (state.canSeeAll && error?.code === "failed-precondition") snapshot = await getDocs(collection(db, DASHBOARD_CONFIG.collections.activities)); else throw error;
        }
        ...
        console.error("Erro ao carregar:", error); setSyncStatus("error", "Falha ao atualizar"); showToast(errorMessage(error), "error");
    } finally {
        if (sessionVersion === state.sessionVersion) { state.activitiesLoading = false; if (!silent) setLoading(false); }
    }
}

function applyFilters({ resetPage = false } = {}) {
    const filters = {
        professionalId: $("filtro-profissional").value, status: $("filtro-status").value,
        type: normalizeText($("filtro-tipo").value), clientSearch: normalizeText($("filtro-cliente").value)
    };
...
function renderDashboard() { renderKpis(); renderCharts(); renderMap(); renderRanking(); renderTable(); }

function renderKpis() {
    const acts = state.filteredActivities;
    const total = acts.length; const completed = acts.filter(a => a.status === "Concluída").length;
    const inProgress = acts.filter(a => a.status === "Em andamento").length; const pending = acts.filter(a => a.status === "Pendente").length;
    ...
    $("kpi-total").textContent = formatNumber(total); $("kpi-total-sub").textContent = `${formatNumber(clients)} ${clients === 1 ? "cliente" : "clientes"}`;
    $("kpi-concluidas").textContent = formatNumber(completed); $("kpi-concluidas-sub").textContent = `${total ? Math.round((completed / total) * 100) : 0}% de conclusão`;
    $("kpi-andamento").textContent = formatNumber(inProgress); $("kpi-andamento-sub").textContent = inProgress ? "Acompanhamento em tempo real" : "Nenhuma visita aberta";
    $("kpi-pendentes").textContent = formatNumber(pending); $("kpi-pendentes-sub").textContent = pending === 1 ? "Visita agendada" : "Visitas agendadas";
    $("kpi-treinamentos").textContent = formatNumber(trainings); $("kpi-treinamentos-sub").textContent = `${total ? Math.round((trainings / total) * 100) : 0}% das atividades`;
    $("kpi-duracao").textContent = formatDuration(avgDur); $("kpi-duracao-sub").textContent = durs.length ? `${formatNumber(durs.length)} visitas concluídas` : "Sem duração";
}

function clearCharts() { for (const c of Object.values(state.charts)) try { c?.destroy(); } catch (e) {} state.charts = {}; }
function destroyChart(name) { try { state.charts[name]?.destroy(); } catch (e) {} delete state.charts[name]; }
function chartBaseOptions() { return { responsive: true, maintainAspectRatio: false, animation: { duration: 350 }, interaction: { intersect: false, mode: "index" }, plugins: { legend: { position: "bottom", labels: { usePointStyle: true, boxWidth: 8, padding: 18, color: "#646779", font: { family: "Outfit", size: 11, weight: "600" } } }, tooltip: { backgroundColor: "#040438", padding: 11, titleFont: { family: "Outfit", weight: "700" }, bodyFont: { family: "Outfit" }, displayColors: true } }, scales: { x: { grid: { display: false }, ticks: { color: "#7a7d8d", font: { family: "Outfit", size: 10 } } }, y: { beginAtZero: true, grid: { color: "rgba(111,114,130,0.10)" }, ticks: { precision: 0, color: "#7a7d8d", font: { family: "Outfit", size: 10 } } } } }; }

function renderCharts() {
    if (typeof window.Chart !== "function") return;
    renderEvolutionChart(); renderStatusChart(); renderTeamChart();
}

function renderEvolutionChart() {
    destroyChart("evolution"); const canvas = $("grafico-evolucao"); const empty = $("grafico-evolucao-vazio");
    if (!state.filteredActivities.length) { canvas.hidden = true; empty.hidden = false; return; }
    ...
    for (const a of state.filteredActivities) {
        ...
        if (a.status === "Concluída") b.completed++; else if (a.status === "Em andamento") b.inProgress++; else if (a.status === "Pendente") b.pending++;
    }
    const ordered = [...buckets.values()].sort((a, b) => a.date - b.date);
    const options = chartBaseOptions(); options.scales.x.stacked = true; options.scales.y.stacked = true; options.scales.x.ticks.maxRotation = 0;
    state.charts.evolution = new window.Chart(canvas, {
        type: "bar",
        data: { labels: ordered.map(i => formatCompactDate(i.date)), datasets: [ { label: "Concluídas", data: ordered.map(i => i.completed), backgroundColor: "rgba(12,155,105,0.86)", borderRadius: 4 }, { label: "Em andamento", data: ordered.map(i => i.inProgress), backgroundColor: "rgba(245,30,48,0.86)", borderRadius: 4 }, { label: "Pendentes", data: ordered.map(i => i.pending), backgroundColor: "rgba(220,140,0,0.82)", borderRadius: 4 } ] },
        options
    });
}

function renderStatusChart() {
    destroyChart("status"); const canvas = $("grafico-status"); const empty = $("grafico-status-vazio");
    if (!state.filteredActivities.length) { canvas.hidden = true; empty.hidden = false; return; }
    canvas.hidden = false; empty.hidden = true;
    ...
    state.charts.status = new window.Chart(canvas, {
        type: "doughnut",
        data: { labels, datasets: [{ data: labels.map(l => counts.get(l) || 0), backgroundColor: labels.map(statusColor), borderColor: "#ffffff", borderWidth: 4 }] },
...
function renderTeamChart() {
    destroyChart("team"); const canvas = $("grafico-equipe"); const empty = $("grafico-equipe-vazio");
    const stats = teamStatistics().slice(0, 12);
    if (!stats.length) { canvas.hidden = true; empty.hidden = false; return; }
    canvas.hidden = false; empty.hidden = true;
    const options = chartBaseOptions(); options.indexAxis = "y"; options.plugins.legend.display = false;
    state.charts.team = new window.Chart(canvas, { type: "bar", data: { labels: stats.map(i => i.professional.name), datasets: [{ label: "Atividades", data: stats.map(i => i.total), backgroundColor: "rgba(4,4,56,0.86)", borderRadius: 7 }] }, options });
}
...
function clearMapLayers() {
    ...
    for (const layer of state.mapLayers) try { state.map.removeLayer(layer); } catch (e) {}
    state.mapLayers = [];
}
function renderMap() {
    initializeMap();
    ...
    if (!activities.length) { state.map.setView(DASHBOARD_CONFIG.defaultMapCenter, DASHBOARD_CONFIG.defaultMapZoom); $("dashboard-btn-google-maps").disabled = true; return; }
    ...
    if (mode === "calor" && typeof window.L.heatLayer === "function") {
        ...
        const layer = window.L.heatLayer(heat, { radius: 27, blur: 22, maxZoom: 14 }).addTo(state.map); state.mapLayers.push(layer);
    } else {
        for (const a of activities) {
            ...
            m.addTo(state.map); state.mapLayers.push(m);
        }
    }
    if (allCoords.length === 1) state.map.setView(allCoords[0], 14); else state.map.fitBounds(window.L.latLngBounds(allCoords), { padding: [28, 28], maxZoom: 14 });
...
function renderRanking() {
    const container = $("ranking-equipe"); const empty = $("ranking-vazio"); const stats = teamStatistics();
    container.replaceChildren();
    if (!stats.length) { empty.hidden = false; return; }
    empty.hidden = true;
    stats.slice(0, 12).forEach((item, index) => {
        const btn = document.createElement("button"); btn.type = "button"; btn.className = "ranking-item";
        btn.innerHTML = `<span class="ranking-position">${index + 1}</span><span class="ranking-main"><strong>${escapeHtml(item.professional.name)}</strong><span>${formatNumber(item.completed)} concluídas · ${formatNumber(item.clientCount)} clientes</span></span><span class="ranking-score"><strong>${formatNumber(item.total)}</strong><small>atividades</small></span>`;
...
function renderTable() {
    const body = $("dashboard-table-body"); const empty = $("table-empty");
    const count = state.filteredActivities.length; const totalPages = Math.max(1, Math.ceil(count / DASHBOARD_CONFIG.pageSize));
        ...
        tr.innerHTML = `
            ...
            <td>
                <div class="table-actions-group">
                    <button class="table-action btn-view" type="button" title="Ver detalhes"><svg viewBox="0 0 24 24"><path d="M1.5 12s3.5-6 10.5-6 10.5 6 10.5 6-3.5 6-10.5 6S1.5 12 1.5 12Z"></path><circle cx="12" cy="12" r="3"></circle></svg></button>
                    ${state.canSeeAll ? `<button class="table-action delete btn-delete" type="button" title="Excluir"><svg viewBox="0 0 24 24"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2M10 11v6M14 11v6"></path></svg></button>` : ''}
                </div>
            </td>
        `;
...
window.deletarAtividadeAdmin = async function(id, rawData) {
    if(confirm("Deseja realmente excluir esta visita e movê-la para a lixeira?")) {
        ...
        } catch(e) { showToast("Erro ao excluir visita.", "error"); }
    }
};

function openModal({ eyebrow, title, content }) {
    $("modal-eyebrow").textContent = eyebrow; $("modal-title").textContent = title; $("modal-content").innerHTML = content;
    const modal = $("dashboard-modal"); if (typeof modal.showModal === "function") modal.showModal(); else modal.setAttribute("open", "");
}
function closeModal() {
    const modal = $("dashboard-modal"); if (typeof modal.close === "function" && modal.open) modal.close(); else modal.removeAttribute("open");
}
...
async function openActivityDetails(activityId) {
    const a = state.activities.find(i => i.id === activityId); if (!a) return;
    openModal({ eyebrow: "Carregando", title: a.client.name, content: `<p>Buscando detalhes...</p>` });

    let reportText = "Não há relatório."; let reportCode = "";
    if (a.reportId) {
        try {
            const snap = await getDoc(doc(db, DASHBOARD_CONFIG.collections.reports, a.reportId));
            if (snap.exists()) { reportText = safeString(snap.data().textoAtual ?? snap.data().texto); reportCode = snap.data().codigo || a.reportId; }
        } catch (e) { reportText = "Erro ao carregar relatório."; }
    }
    const content = `
        <div class="modal-grid">
            ${detailBox("Protocolo", `#${a.id}`)} ${detailBox("Status", a.status)}
            ${detailBox("Profissional", a.professional.name)} ${detailBox("Tipo", a.type)}
            ${detailBox("Agendamento", formatDateTime(a.scheduledAt))} ${detailBox("Duração", formatDuration(a.durationMinutes))}
            ${detailBox("Cliente", a.client.name, true)} ${detailBox("Endereço", a.client.address || a.client.city, true)}
            ${detailBox("Nota da agenda", a.note || "Sem observação.", true)}
            ${detailBox(reportCode ? `Relatório ${reportCode}` : "Relatório", reportText, true)}
        </div>
    `;
    $("modal-eyebrow").textContent = "Detalhes da atividade"; $("modal-title").textContent = a.client.name; $("modal-content").innerHTML = content;
}

function openProfessionalProfile(id) {
    const p = state.professionalMap.get(id); if(!p) return;
    openModal({ eyebrow: "Perfil", title: p.name, content: `<div class="modal-grid">${detailBox("E-mail", p.email)} ${detailBox("Telefone", p.phone)} ${detailBox("Região", p.region)} ${detailBox("Situação", p.status)}</div>` });
}
...
function exportCsv() {
    ...
    const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `visitas.csv`;
...
function initTabs() {
    document.querySelectorAll('.nav-tab').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            document.querySelectorAll('.aba-conteudo').forEach(a => { a.hidden = true; a.classList.remove('active'); });
            btn.classList.add('active');
            const target = $(btn.dataset.target);
            target.hidden = false; target.classList.add('active');
            if (btn.dataset.target === 'aba-dashboard') setTimeout(() => state.map?.invalidateSize(), 50);
        });
    });
}
...
function renderClientsAdminTable() {
    const tbody = $("tabela-clientes-body"); if(!tbody) return;
    let html = '';
    state.clients.forEach(c => {
        html += `<tr>
            <td><span class="cell-primary">${escapeHtml(c.name)}</span><span class="cell-secondary">CNPJ/Doc: ${c.raw.codigoCnpj || '-'}</span></td>
            ...
            <td>
                <div class="table-actions-group">
                    <button class="table-action" onclick="window.editarClienteAdmin('${c.id}')"><svg viewBox="0 0 24 24"><path d="M12 20h9M16.5 3.5a2.12 2.12 0 013 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg></button>
                    ${state.canSeeAll ? `<button class="table-action delete" onclick="window.deletarClienteAdmin('${c.id}')"><svg viewBox="0 0 24 24"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2M10 11v6M14 11v6"></path></svg></button>` : ''}
                </div>
            </td>
        </tr>`;
    });
...
window.editarClienteAdmin = function(id) {
    const c = state.clientMap.get(id); if(!c) return;
    $("mc-id").value = id; $("mc-nome").value = c.name; $("mc-cidade").value = c.city; $("mc-uf").value = c.state; $("mc-endereco").value = c.address;
    $("cliente-modal-title").textContent = "Editar Loja"; $("modal-cliente").showModal();
}

window.deletarClienteAdmin = async function(id) {
    if(confirm("Tem certeza que deseja excluir esta Loja/Cliente do sistema?")) {
        try { await deleteDoc(doc(db, DASHBOARD_CONFIG.collections.clients, id)); showToast("Loja excluída.", "success"); loadReferenceData(state.sessionVersion); } 
...
function renderTeamAdminTable() {
    const tbody = $("tabela-equipe-body"); if(!tbody) return;
    let html = '';
    state.professionals.forEach(p => {
        ...
        html += `<tr>
            <td><span class="cell-primary">${escapeHtml(p.name)}</span><span class="cell-secondary">${escapeHtml(p.phone || '-')}</span></td>
            ...
            <td>
                <div class="table-actions-group">
                    <button class="table-action" onclick="window.editarEquipeAdmin('${p.id}')"><svg viewBox="0 0 24 24"><path d="M12 20h9M16.5 3.5a2.12 2.12 0 013 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg></button>
                    ${state.canSeeAll ? `<button class="table-action delete" onclick="window.deletarEquipeAdmin('${p.sourceCollection}', '${p.id}')"><svg viewBox="0 0 24 24"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2M10 11v6M14 11v6"></path></svg></button>` : ''}
                </div>
            </td>
        </tr>`;
    });
...
window.editarEquipeAdmin = function(id) {
    const p = state.professionalMap.get(id); if(!p) return;
    $("mu-id").value = id; $("mu-colecao-original").value = p.sourceCollection;
    $("mu-nome").value = p.name; $("mu-email").value = p.email; $("mu-perfil").value = p.sourceCollection; 
    $("mu-regiao").value = p.region; $("mu-telefone").value = p.phone;
    $("label-status-equipe").hidden = false; $("mu-status").hidden = false; $("mu-status").value = p.ativo ? "true" : "false";
    $("equipe-modal-title").textContent = "Editar Membro"; $("modal-equipe").showModal();
}
...
window.deletarEquipeAdmin = async function(col, id) {
    if(confirm("Tem certeza que deseja excluir permanentemente este membro do acesso ao app?")) {
        try { await deleteDoc(doc(db, col, id)); showToast("Membro excluído.", "success"); loadReferenceData(state.sessionVersion); } 
        catch (e) { showToast("Erro ao excluir.", "error"); }
    }
}
...
function initBrasilAPI() {
    const iptCnpj = $("mc-cnpj"); const iptCep = $("mc-cep");
    if(iptCnpj) iptCnpj.addEventListener('blur', async () => {
        const c = iptCnpj.value.replace(/\D/g, ''); const st = $("mc-status-cnpj");
        if(c.length === 14) {
            st.textContent = "(Buscando...)"; st.style.color = "var(--blue)";
            try {
                const res = await fetch(`https://brasilapi.com.br/api/cnpj/v1/${c}`);
                if(res.ok) {
                    const d = await res.json();
                    $("mc-nome").value = d.nome_fantasia \vert{}\vert{} d.razao_social; $("mc-cep").value = (d.cep||"").replace(/\D/g, '');
                    $("mc-endereco").value = d.logradouro || ""; $("mc-cidade").value = d.municipio \vert{}\vert{} ""; $("mc-uf").value = d.uf || "";
                    st.textContent = "(Encontrado)"; st.style.color = "var(--green)";
                } else st.textContent = "(Falha)";
            } catch(e) { st.textContent = "(Falha)"; }
        }
    });
    if(iptCep) iptCep.addEventListener('blur', async () => {
        const c = iptCep.value.replace(/\D/g, ''); const st = $("mc-status-cep");
        if(c.length === 8) {
            st.textContent = "(Buscando...)"; st.style.color = "var(--blue)";
            try {
                ...
                if(res.ok) {
                    const d = await res.json();
                    $("mc-endereco").value = d.street || ""; $("mc-cidade").value = d.city \vert{}\vert{} ""; $("mc-uf").value = d.state || "";
                    st.textContent = "(Encontrado)"; st.style.color = "var(--green)";
                } else st.textContent = "(Falha)";
            } catch(e) { st.textContent = "(Falha)"; }
        }
...
function bindEvents() {
    ...
    $("filtro-data-inicial").value = dateInputValue(firstDay); $("filtro-data-final").value = dateInputValue(today);

    $("dashboard-login-form").addEventListener("submit", async e => {
        e.preventDefault();
        const em = $("dashboard-email").value.trim(); const pw = $("dashboard-senha").value; const err = $("dashboard-login-erro");
        err.hidden = true; if (!em || !pw) { err.textContent = "Preencha todos os campos."; err.hidden = false; return; }
        $("dashboard-btn-entrar").disabled = true; $("dashboard-btn-entrar").textContent = "Verificando...";
        try { await signInWithEmailAndPassword(auth, em, pw); } 
        catch (error) { err.textContent = errorMessage(error); err.hidden = false; $("dashboard-btn-entrar").disabled = false; $("dashboard-btn-entrar").textContent = "Entrar"; }
    });

    $("dashboard-btn-sair").addEventListener("click", async () => { if (!state.loading) try { await signOut(auth); } catch (e) {} });
    ...
    $("filtro-cliente").addEventListener("keydown", e => { if (e.key === "Enter") applyRequestedFilters(); });

    $("pagina-anterior").addEventListener("click", () => { if (state.currentPage > 1) { state.currentPage--; renderTable(); } });
    $("pagina-proxima").addEventListener("click", () => { const t = Math.max(1, Math.ceil(state.filteredActivities.length / DASHBOARD_CONFIG.pageSize)); if (state.currentPage < t) { state.currentPage++; renderTable(); } });

    $("modal-fechar").addEventListener("click", closeModal);
    $("dashboard-modal").addEventListener("click", e => { if (e.target === $("dashboard-modal")) closeModal(); });
    
    // CRUD EVENTOS
    initTabs(); initBrasilAPI();
    
    $("btn-abrir-modal-cliente")?.addEventListener("click", () => {
        $("form-cliente-modal").reset(); $("mc-id").value = ""; $("mc-status-cnpj").textContent = ""; $("mc-status-cep").textContent = "";
        $("cliente-modal-title").textContent = "Nova Loja"; $("modal-cliente").showModal();
    });
    $("btn-fechar-cliente")?.addEventListener("click", () => $("modal-cliente").close());
    
    $("form-cliente-modal")?.addEventListener("submit", async e => {
        e.preventDefault(); const btn = $("btn-salvar-modal-cli"); btn.disabled = true; btn.textContent = "A gravar...";
        try {
            const id = $("mc-id").value || "cli_" + Date.now();
            const data = {
                codigoCnpj: $("mc-cnpj").value ? ofuscarCNPJ($("mc-cnpj").value.replace(/\D/g, '')) : null,
                nome: $("mc-nome").value, cidade: $("mc-cidade").value, uf: $("mc-uf").value.toUpperCase(),
                enderecoCompleto: $("mc-endereco").value, status: "Ativo", atualizadoEm: new Date()
            };
            if(!$("mc-id").value) data.criadoEm = new Date();
            await setDoc(doc(db, DASHBOARD_CONFIG.collections.clients, id), data, {merge: true});
            showToast("Loja salva com sucesso!", "success"); $("modal-cliente").close(); loadReferenceData(state.sessionVersion);
        } catch(err) { showToast("Erro ao salvar", "error"); } finally { btn.disabled = false; btn.textContent = "Salvar Loja"; }
    });

    $("btn-abrir-modal-equipe")?.addEventListener("click", () => {
        $("form-equipe-modal").reset(); $("mu-id").value = ""; $("label-status-equipe").hidden = true; $("mu-status").hidden = true;
        $("equipe-modal-title").textContent = "Novo Membro"; $("modal-equipe").showModal();
    });
    $("btn-fechar-equipe")?.addEventListener("click", () => $("modal-equipe").close());

    $("form-equipe-modal")?.addEventListener("submit", async e => {
        e.preventDefault(); const btn = $("btn-salvar-modal-user"); btn.disabled = true; btn.textContent = "A gravar...";
        try {
            const col = $("mu-perfil").value; const oldCol = $("mu-colecao-original").value;
            const id = $("mu-id").value || (col==='promotores'?'ptv_':(col==='assistencia'?'ast_':'adm_')) + Date.now();
            const role = col==='promotores'?'Promotor Técnico':(col==='assistencia'?'Assistente Técnico':'Administrador');
            
            const data = { nome: $("mu-nome").value, email: $("mu-email").value, perfil: role, regiao: $("mu-regiao").value, telefone: $("mu-telefone").value, atualizadoEm: new Date() };
            if(!$("mu-id").value) { data.ativo = true; data.criadoEm = new Date(); } else { data.ativo = $("mu-status").value === "true"; }
            
            if($("mu-id").value && oldCol && oldCol !== col) { await deleteDoc(doc(db, oldCol, id)); } // Mudou de perfil
            await setDoc(doc(db, col, id), data, {merge: true});
            showToast("Equipe atualizada!", "success"); $("modal-equipe").close(); loadReferenceData(state.sessionVersion);
        } catch(err) { showToast("Erro ao salvar", "error"); } finally { btn.disabled = false; btn.textContent = "Cadastrar Membro"; }
    });

    $("form-nova-visita")?.addEventListener("submit", async e => {
        e.preventDefault(); const btn = $("btn-salvar-visita"); btn.disabled = true; btn.textContent = "Agendando...";
        try {
            const id = "atv_" + Date.now();
            await setDoc(doc(db, DASHBOARD_CONFIG.collections.activities, id), {
                clienteId: $("v-cliente").value, ptvId: $("v-responsavel").value,
                data: new Date(`${$("v-data").value}T${$("v-hora").value}:00`),
                objetivo: $("v-motivo").value, nota: $("v-nota").value,
                status: "Pendente", tipo: "Visita", criadoEm: new Date(), atualizadoEm: new Date()
            });
            showToast("Visita agendada!", "success"); $("form-nova-visita").reset(); loadActivities().catch(()=>{});
        } catch(err) { showToast("Erro ao agendar.", "error"); } finally { btn.disabled = false; btn.textContent = "Atribuir Visita"; }
    });
}
...
async function applyRequestedFilters() {
    try {
        ...
        if (!state.loadedRange || state.loadedRange.startValue !== r.startValue || state.loadedRange.endValue !== r.endValue) await loadActivities();
...
function clearFilters() {
    const today = new Date(); const firstDay = new Date(today.getFullYear(), today.getMonth(), 1);
    $("filtro-data-inicial").value = dateInputValue(firstDay); $("filtro-data-final").value = dateInputValue(today);
    $("filtro-profissional").value = ""; $("filtro-status").value = ""; $("filtro-tipo").value = ""; $("filtro-cliente").value = "";
    state.currentPage = 1; loadActivities().catch(() => {});
}

bindEvents();

onAuthStateChanged(auth, user => {
    if (!user) { ++state.sessionVersion; state.user = null; resetStateForSession(); showLogin(); setSyncStatus("", "Aguardando login"); return; }
    handleAuthenticatedUser(user).catch(e => { showToast(errorMessage(e), "error"); });
});

async function handleAuthenticatedUser(user) {
    const v = ++state.sessionVersion; resetStateForSession(); state.user = user; showDashboard();
    setLoading(true, "Validando acesso..."); setSyncStatus("loading", "Validando acesso...");
    try {
        const access = await resolveUserAccess(user); if (v !== state.sessionVersion) return;
        state.profile = access.profile; state.canSeeAll = access.canSeeAll;
        $("dashboard-user-name").textContent = state.profile.name; $("dashboard-user-role").textContent = state.canSeeAll ? "Visão de gestão" : "Visão individual";
        await loadReferenceData(v); if (v !== state.sessionVersion) return;
        await loadActivities();
        clearInterval(state.autoRefreshTimer); state.autoRefreshTimer = setInterval(() => { if (!document.hidden && state.user && !state.loading) loadActivities({ silent: true }).catch(()=>{}); }, DASHBOARD_CONFIG.autoRefreshMs);
    } catch (e) { showToast(errorMessage(e), "error"); try { await signOut(auth); } catch(ex){} } 
    finally { if (v === state.sessionVersion) setLoading(false); $("dashboard-btn-entrar").disabled = false; $("dashboard-btn-entrar").textContent = "Entrar"; }
}
